...
In the implementation we impose that the normalization of the measures is related to the number of samples taken into account. For instance, if distribution 1 shoudl hold double the mass of distribution 1, then the former could be approximated by e.g. 200 data points, while the latter one only counts 100 data points. For the input via the txt file this is simply the number of individual data points contained in the data set. If the user handed in an analytical expressions for  distributions the number of samples must be specified for each distribution.
