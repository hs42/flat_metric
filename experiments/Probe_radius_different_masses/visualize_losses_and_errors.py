...
plot_uncertainties_par = {False: 0, True: 1}[plot_uncertainties]

def init():
    root = tk.Tk()
    root.withdraw()
    path = askdirectory(title='Select folder containing directories of \'Probe radius with different masses\'-experiments')
...
def plot(distance_estimates, relative_errors, actual_penalties, r_values, m_values, n_values):
    ...
    x_label_list = unique_r[::k]
    y_label_list = ratio_n_to_m[::k]
    ax2.set_xticks(np.arange(len(unique_r))[::k])
