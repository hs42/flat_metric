...
for i, dir in enumerate(dirs):
    ...
    flat_metric[i,0] = -np.mean(distance_train[:-50:-1,1]) #mean of last 10 entries
    flat_metric[i,1] = np.std(distance_train[:-50:-1,1]) #mean of last 10 entries
...
plt.errorbar(r, flat_metric[:,0], yerr=3*flat_metric[:,1], fmt=".", color='r',ms=13, 
    label='experiment', elinewidth=2)
plt.plot(truth_x, truth_y, label='ground truth', linewidth=3)
plt.ylim([-0.1,2.3])
plt.legend()
plt.xlabel(r'radius r of distribution $\delta_r$')
plt.ylabel(r'$W_1^{1,1}(N\delta_0, N\delta_r)$')
