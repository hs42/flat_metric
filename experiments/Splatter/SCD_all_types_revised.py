...
import sys
sys.path.append('.{s}lnets{s}tasks{s}dualnets{s}mains'.format(s=os.sep))
...
cell_types = ['Group1', 'Group2', 'Group3', 'Group4', 'Group5']
...
for type1 in cell_types:
    for type2 in cell_types:
        ...
        distance_std = np.std(distance_train[:-data_points_to_consider:-1,1])
